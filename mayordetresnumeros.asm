...
main:
#como vamos a solicitar 3 valores los vamos a requerir para almacear y por último comparar para indicar cual el numero mayor
...
#COmando sge nos trae la información del mayor número
sge $t3 $t1 $t0		# realiza la comparación entre el dato almacenados en t0 y t1 (si t1 es mayor o igual a t0)
...
elseB:
slt $t3 $t0 $t1		# realiza la comparación entre el dato almacenados en t1 y t0 (si t1 es menor el valor en t0)
beq $t3 $zero elseC
sge $t3 $t2 $t1
##beq $t3 $zero elseC
move $a0 $t1
